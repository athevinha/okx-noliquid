...
describe('OKX convert size to contract', () => {
    ...
    before(async () => {
        supportFutureCryptos = [...(await getSupportCrypto({})), ...(await getSupportCrypto({})), ...(await getSupportCrypto({}))]
        supportFutureCryptosByInstId = supportFutureCryptos.map(e => e.instId)
    })
    it('Can fetch multi contract (Future) size', async () => {
        let contract = await Promise.all(supportFutureCryptosByInstId.map(async spCrypto => {
