  ...
  bot.command("positions", async (ctx) => {
    try {
      ...
      positions.forEach(position => {
        ...
        let report = `<b>[${position.posSide.toUpperCase()}]</b> <b><a href="${tradeLink}">${position.instId.split('-').slice(0,2).join('/')}</a></b> (<code>$${zerofy(position.notionalUsd)}</code>)\n`;
        report += `• <b>Avg Entry:</b> <code>$${zerofy(position.avgPx)}</code>\n`;
        report += `• <b>Margin Ratio:</b> <code>${zerofy(position.mgnRatio)}</code>%\n`;
        report += `• <b>Leverage:</b> <code>${zerofy(position.lever)}</code>x\n`;
        report += `• <b>PnL:</b> <code>${zerofy(Number(position.uplRatio) * 100)}</code>% (<code>$${zerofy(position.upl)}</code>) • ${pnlIcon}\n`;
        report += `• <b>Realized Pnl:</b> <code>$${zerofy(realizedPnl)}</code> • ${realizedPnlIcon}\n`;
        positionReports += report;
        totalPnl += parseFloat(position.upl);
        totalRealizedPnl += realizedPnl;
      });
      positionReports += `<code>-------------------------------</code>\n`;
      positionReports += `<b>Est. PnL:</b> <code>$${zerofy(totalPnl)}</code> • ${totalPnl >= 0 ? "🟩" : "🟥"}\n`;
      positionReports += `<b>Est. Realized PnL:</b> <code>$${zerofy(totalRealizedPnl)}</code> • ${totalRealizedPnl >= 0 ? "🟩" : "🟥"}\n`;
